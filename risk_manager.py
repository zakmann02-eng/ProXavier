...
class XavierConfig:
    ...
    # ── Per-trade risk ─────────────────────────────────────────────────────────
    risk_pct: float            # % of equity risked per trade (default 1.0)
    allow_short: bool
    min_rr_ratio: float        # minimum reward:risk ratio to enter
    orb_minutes: int           # opening range window in minutes
    min_price: float           # minimum stock price to trade

    # ── Position limits ────────────────────────────────────────────────────────
    max_positions: int         # total open positions allowed
    max_day_positions: int     # day-trade positions allowed at once
    max_swing_positions: int   # swing positions allowed at once

    # ── Portfolio-level risk gates ─────────────────────────────────────────────
    max_portfolio_risk_pct: float   # halt when total open risk exceeds this %
    daily_loss_limit_pct: float     # halt trading for the day if daily loss > this %
    consecutive_loss_halt: int      # halt after N consecutive losing trades
    max_crypto_alloc_pct: float     # max % of equity allocated to crypto positions
