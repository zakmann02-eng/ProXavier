...
Scoring (0–100) reflects conviction: the more confirming signals, the higher
the score. Only signals above a per-strategy minimum score are returned.
"""
...
def _size_equity(equity: float, risk_pct: float, entry: float, stop: float) -> float:
    """Integer share count sized to risk a fixed % of equity."""
...
def _size_crypto(
    ...
    """Fractional crypto qty — risk-sized but capped so notional ≤ max_notional_pct of equity."""
    risk_usd    = equity * (risk_pct / 100.0)
    risk_per_unit = abs(entry - stop)
    if risk_per_unit < 0.0001 or entry <= 0:
        return 0.0
    qty = risk_usd / risk_per_unit
    # Hard notional cap: don't put more than max_notional_pct of equity into one crypto pos
...
class ORBStrategy:
    """
    Capture the first N minutes of trading as the "opening range".
    Fire LONG on a clean breakout above ORB high (SHORT below ORB low if shorts enabled).
    Require at least one additional confirmation besides the breakout itself.

    Score (max 100):
        ORB_BREAK        +40  (required)
        EMA crossover    +15
        RSI in momentum  +15
        Price > VWAP     +10
        Volume surge     +10
    ...
    STRATEGY_ID = "ORB"
    MIN_SCORE   = 55  # minimum: ORB + 1 confirmation
...
class VWAPReversionStrategy:
    """
    Fade extreme deviations from VWAP back to the mean.
    Works mid-session (10:30 AM – 2:30 PM ET) when the trend is established.

    Score (max 100):
        Deviation ≥ 1.5 ATR from VWAP  +30  (required)
        Deviation ≥ 2.0 ATR             +10  (bonus)
        RSI oversold/overbought         +20
        Bollinger Band touch            +15
        Volume exhaustion (declining)   +15
        Volume uptick (reversal candle) +10
    """

    STRATEGY_ID   = "VWAP_REV"
    MIN_SCORE     = 50
    MIN_DEVIATION = 1.5   # ATR multiples below/above VWAP to trigger
    ...
    def evaluate(
        ...
        # Only trade mean reversion mid-session
        in_window = (
...
# ──────────────────────────────────────────────────────────────────────────────
# Strategy 3: Momentum Breakout (Swing)
# ──────────────────────────────────────────────────────────────────────────────

class MomentumStrategy:
    """
    Swing trade on a confirmed 20-day high/low breakout with ADX and volume.
    Positions hold overnight; EOD close is suppressed for swing trades.

    Score (max 100):
        20-day H/L break   +40  (required)
        ADX > 25           +20
        Volume surge ≥1.5x +20
        Price > EMA50      +10
        RSI in 55–75       +10
    """
...
# ──────────────────────────────────────────────────────────────────────────────
# Strategy 4: ETF Sector Rotation
# ──────────────────────────────────────────────────────────────────────────────

class ETFRotationStrategy:
    """
    Rank sector ETFs by 5-day relative momentum and enter the top performers.
    Swing trade — holds multiple days until momentum decays.

    Score (max 100):
        Rank #1 among peers  +40  (rank #2  +30)
        Above EMA50          +20
        5-day momentum > 2%  +20
        RSI > 55             +10
        Price rising past 5d +10
    """
    ...
    def update_rankings(self, symbol_dfs: dict[str, pd.DataFrame]) -> None:
        """Compute 5-day momentum for each ETF. Call before evaluate()."""
...
# ──────────────────────────────────────────────────────────────────────────────
# Strategy 5: Crypto Trend
# ──────────────────────────────────────────────────────────────────────────────

class CryptoTrendStrategy:
    """
    Multi-timeframe crypto trend following.
    Requires daily trend alignment (EMA20 > EMA50) AND intraday momentum.
    Enters on pullback re-test of intraday EMA levels.

    Score (max 100):
        EMA bull/bear on 1m   +20  (required)
        EMA trend on daily    +20  (required)
        RSI in momentum zone  +15
        Price vs VWAP         +15
        MACD bull/bear        +15
        Volume surge          +15
    """
    ...
    def evaluate(
        ...
        if daily_bull:
            if ema9_v > ema20_v:
                ...
                return None  # intraday not aligned
        ...
        # Classify as swing if daily trend is strong, else day
